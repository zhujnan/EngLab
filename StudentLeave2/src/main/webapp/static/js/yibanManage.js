$(function(){
     $.ajax({
     		async: true,
          type: "post",
          url: "../task/unFinishedList.do",//向后台发送请求
          dataType: "json",
          data: {userId:$("#ume").val(),groupId:$("#group").val(),page:'1',rows:'5'},
          cache: false,
...
for (var i = 0; i <data.rows.length; i++) {//拼接对应<th>需要的值
    				...
    				trs+="<tr ><td >"+ data.rows[i].id
				        ...
				        +")'>流程执行过程</a><a  href='javascript:;' onclick='openListCommentDialog("+data.rows[i].id+")'>历史批注</a><a target='_blank' href='../task/showHisCurrentView.do?taskId="
...
var pageCount = data.total/5; //总页数
					        	  var options = {
					                  ...
					                  onPageClicked: function (event, originalEvent, type, page) {
					                      $.ajax({
					                            async: true,
					                            url: "../task/unFinishedList.do",
					                            type: "post",
					                            dataType : "json",
					                            data: {userId:$("#ume").val(),groupId:$("#group").val(),page:page,rows:'5'},
					                            cache: false,
					                            success: function (data) {
					                          	 $('tbody').empty();
